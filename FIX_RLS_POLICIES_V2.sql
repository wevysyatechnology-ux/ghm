/*
  ...
  URGENT FIX - This version works without auth schema permissions!
...
-- =============================================================================
-- STEP 2: Create helper functions in PUBLIC schema (not auth)
-- =============================================================================

-- Drop old functions if they exist
DROP FUNCTION IF EXISTS public.get_user_role();
DROP FUNCTION IF EXISTS public.get_user_approval_status();
...
-- ============================================
-- PROFILES TABLE - Most important!
-- ============================================

-- Everyone can view all profiles (read-only, safe)
CREATE POLICY "Allow all authenticated users to view profiles"
  ON profiles FOR SELECT
  TO authenticated
  USING (true);

-- Users can update their own profile (but not role/approval_status)
-- This uses a subquery but it's safe because we're not checking profiles recursively
...
CREATE POLICY "Users can insert own profile on signup"
  ...
  WITH CHECK (
    id = auth.uid()
    -- New users start as regular users, pending approval
    AND role = 'user'
...
-- =============================================================================
-- STEP 4: Ensure admin user exists and is approved
-- =============================================================================
...
BEGIN
  ...
  IF admin_exists THEN
    ...
    -- Make sure profile exists and is approved
    INSERT INTO profiles (id, email, full_name, role, approval_status, phone)
    VALUES (
      admin_user_id,
      'admin@wevysya.com',
      'Super Admin',
  ...
  ELSE
    ...
    RAISE NOTICE '⚠️  You need to create it first. Run SETUP_DATABASE.sql or create manually.';
  END IF;
END $$;
...
BEGIN
  ...
  RAISE NOTICE '  - Approved admins: %', approved_admin_count;
  RAISE NOTICE '';
  ...
  ELSE
    RAISE NOTICE '⚠️  No approved admin found.';
    RAISE NOTICE '⚠️  Run SETUP_DATABASE.sql to create admin user.';
  END IF;
